Features
This document describes the feature set used by the RTAT prediction model: 38 numeric model features, plus one categorical feature (parts_shipping_tier) that is documented and leakage-audited but held out of the numeric model. Each feature is documented with:
...
POST_CLOSE	Value is only available after the repair completes (actual delivery days, repair duration, parts arrival).	🔴 Not deployment-safe — excluded from MODEL_FEATURES
The pipeline includes the POST_CLOSE parts features in the dataset for audit reference (parts_order_to_arrival_days_safe, its binned twin parts_delivery_tier, and parts_has_arrival_flag) and explicitly excludes all three from MODEL_FEATURES — each requires parts to have actually arrived, which is not known at intake scoring. Their deployment-safe replacement is seg_delivery_days_hist, a segment-level historical median computed on training data only.

Feature spec at a glance
MODEL_FEATURES (the leakage-reviewed list in feature_engineering.py) contains 39 entries split into eight groups defined in the FEATURE_SPEC constant. Of these, 38 are numeric and feed the trained LightGBM/XGBoost models; the one categorical-string column (parts_shipping_tier) is documented and audited but excluded from the numeric model:
...
DMS	9	ordered_via_dms, parts_line_count, parts_delivery_tier_known, seg_delivery_days_hist
Interactions	3	geo_channel_risk, rural_parts_flag, eng_channel_risk
Total in MODEL_FEATURES	39	38 numeric + 1 categorical (parts_shipping_tier). parts_order_to_arrival_days_safe, parts_delivery_tier, and parts_has_arrival_flag are in the dataset but excluded from MODEL_FEATURES entirely.
The two-track CORE / EXTENDED design splits the 38 numeric features by missingness pattern: CORE (31 features) is low-missingness and safe for linear models with median fill; EXTENDED (the full 38 numeric) adds DMS-dependent features (~75% missingness) for tree models that handle nulls natively.
...
Rationale: City has ~13,900 unique values; State has 56 (50 states plus DC and US territories). Without smoothing, low-count cities would memorize individual training observations and overfit. Smoothing also is a standard mitigation for target-encoding leakage.
...
7. DMS features
9 in-model features, all dependent on DMS being the parts dispatch channel. ~75% of repairs have DMS feature nulls because DMS routes only ~25% of all repairs. Tree models handle these nulls natively; linear models can't (which is why CORE excludes them). Three further DMS-derived columns (parts_order_to_arrival_days_safe, parts_delivery_tier, parts_has_arrival_flag) are POST_CLOSE and excluded from the model — see "Delivery speed" below and the EXCLUDED block in the summary table.
...
parts_has_arrival_flag was previously listed here as POST_INTAKE, but it is only set once a part has actually arrived, which for the repair being scored is part of the turnaround being predicted. It is reclassified POST_CLOSE and excluded from MODEL_FEATURES (see the EXCLUDED block at the end).

Delivery speed — the carefully-designed pair
This is the single most important design decision in DMS features. The pipeline includes delivery features with different deployment profiles:
...
Timing: POST_CLOSE — the value only exists once the parts have actually arrived, which is unavailable at intake scoring.
...
parts_delivery_tier (EXCLUDED) and parts_delivery_tier_known
parts_delivery_tier bins parts_order_to_arrival_days_safe into 6 tiers (1=fastest, 6=slowest). It inherits the same POST_CLOSE timing as the continuous value it is derived from, and is therefore excluded from MODEL_FEATURES (it was the binned twin that an earlier version left in the model; the leakage audit flags it POST_CLOSE). The model relies on seg_delivery_days_hist for the analogous segment-level delivery signal.

parts_delivery_tier_known is the administrative presence flag (1 if tier is populated, ~15% positive). It stays in the model: it is just a presence indicator, audited clean, and is the borderline case to revisit if a fully intake-only model is required.
...
The 38 numeric features below feed the trained LightGBM/XGBoost models. The one categorical entry (parts_shipping_tier, #33) is part of MODEL_FEATURES and is leakage-audited, but is held out of the numeric model — the boosters here train on the 38 numeric columns.
...
32	parts_has_shipment_flag	DMS	POST_INTAKE	✅
33	parts_shipping_tier	DMS	POST_INTAKE	✅ (categorical — audited, held out of the numeric model)
34	parts_delivery_tier_known	DMS	POST_INTAKE	✅
35	seg_delivery_days_hist	DMS	TRAINING	✅
36	parts_truncation_flag	DMS	INTAKE	✅
37	geo_channel_risk	Interactions	INTAKE	✅
38	rural_parts_flag	Interactions	INTAKE	✅
39	eng_channel_risk	Interactions	MEDIUM	⚠ Verify
The 38 numeric model features are rows 1-32 and 34-39 (every row above except the categorical parts_shipping_tier at #33).

Plus, in the dataset but EXCLUDED from MODEL_FEATURES (all POST_CLOSE — require parts to have arrived):

| - | parts_order_to_arrival_days_safe | DMS | POST_CLOSE | 🔴 No | | - | parts_delivery_tier | DMS | POST_CLOSE (binned twin) | 🔴 No | | - | parts_has_arrival_flag | DMS | POST_CLOSE | 🔴 No |

References to other documentation
Production code: feature_engineering.py — every transformation here is documented in the corresponding subsection (6A through 6O). The _EXCLUDED_FROM_MODEL frozenset there is the single source of truth for the three post-close parts features held out of the model.

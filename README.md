...
- **Predictions only matter if they translate to interventions.** A 4.74-day MAE is operationally meaningless without saying which 28 Market_Category × Channel segments to act on and which of four operational levers to pull. The downstream lever decomposition is the deliverable; the model is the input.

This project addresses all three within a reproducible single-machine pipeline.

The pipeline processes **2.19M repair records** across 4 source years, joins three operational tables (master records, parts ledger, callback records), engineers a leakage-reviewed feature set (**38 numeric model features**, plus one categorical feature documented but held out of the numeric model), trains and compares **15 candidate models** (7 classifiers + 8 regressors), and produces a prioritized action plan for the worst-performing service segments.
...
| Metric | Value | Note |
|---|---|---|
| **Holdout AUC at T=5** | **0.806** | LightGBM (XGBoost 0.803); 38 features, 2026 holdout |
| **Holdout F1 at T=5** | **0.722** | At decision threshold 0.50 |
| **Holdout MAE** | **4.74 days** | 30% improvement over the 6.77-day mean baseline |
| Holdout R² | 0.34 | |
...
│   ├── feature_engineering.py    Step 6: 38 numeric features (+1 categorical) + leakage review
...
| 6. Random Forest | 5.06 | 9.20 | 0.298 |
| 7. XGBoost | 5.04 | 9.13 | 0.308 |
| 8. LightGBM | 5.07 | 9.27 | 0.287 |

Lasso zeroed 4 of 31 CORE features at α=0.01 — modest regularization. On validation regression the tree models cluster tightly and XGBoost (5.04) edges LightGBM (5.07) within noise. The final LightGBM model, refit on all 2023-2025, reaches **4.74 MAE** on the 2026 holdout (RMSE 8.49, R² 0.34).
...
| 5. Random Forest | 0.785 | 0.705 | 0.660 | 0.755 |
| 6. XGBoost | 0.794 | 0.709 | 0.683 | 0.737 |
| 7. LightGBM | 0.762 | 0.692 | 0.673 | 0.713 |

On this single-fit validation bake-off XGBoost leads on AUC (0.794); LightGBM (0.762, best_iter 114) trails on the single fit but pulls ahead on the final refit holdout (see below). LightGBM is the primary model — it leads at T=5 and on regression on the deployable holdout.
...
|---|---|---|---|---|
| T=3 (strict) | 0.753 | 0.798 | 0.506 | 27.9% positive |
| **T=5 (primary)** | **0.762** | **0.806** | **0.722** | 46.8% positive, balanced |
| T=7 (relaxed) | 0.798 | 0.815 | 0.831 | 62.3% positive |
| T=10 (loose) | 0.818 | 0.841 | 0.900 | 75.4% positive |

Holdout regression MAE: **4.74 days** (RMSE 8.49, R² 0.34). The holdout AUC sits at or above the selection-fold validation at every threshold because the final model is refit on the extra 2025 year before scoring 2026 — the consistent across-threshold pattern, not a single tight gap, is the generalization evidence. XGBoost is within ~0.01 AUC of LightGBM across thresholds (holdout 0.795/0.803/0.816/0.832); LightGBM leads at T=3/T=5/T=10 and XGBoost edges it at T=7.

## Feature audit summary

Of the 39 audited features (38 numeric model features + 1 categorical held out of the numeric model), the automated 5-test audit returns:
...
| ⚠ STABILITY FLAG | 3 | `month_of_year`, `quarter`, `month_mean_rtat` — 2026 holdout (partial year) shifts distribution from 2023-2025 training |

The two POST_CLOSE parts features that earlier reviews flagged (`parts_delivery_tier`, `parts_has_arrival_flag`) are **no longer model features** — they were removed from `MODEL_FEATURES`, with the training-cohort `seg_delivery_days_hist` carrying the deployment-safe delivery signal in their place. The three "STABILITY FLAG" features are flagged for human review, not for removal — the train-vs-holdout shift is by design (the holdout is a temporal slice covering Jan-Feb 2026, while training spans full calendar years). See [`docs/methodology.md`](docs/methodology.md) for the full leakage discipline.
...
|---|---|---|
| Very low (0-20%) | **69.0%** | **19.7%** |
| Very high (80-100%) | **57.5%** | **28.0%** |
| **Gap** | **11.5pp** | **8.3pp** |

Predicted lateness drives an 11.5-point promoter gap and 8.3-point detractor gap — confirming RTAT optimization addresses a real customer-experience problem, not just an operational metric. The signal cannot be tuned from inside the pipeline because NPS is never a feature. Full breakdown across all five buckets: `outputs/prioritization/nps_validation.csv`, 42,451 responders.
